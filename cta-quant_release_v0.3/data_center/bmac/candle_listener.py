...
import logging
import pandas as pd
from data_center.bmac.api.binance.binance_market_ws import (
    get_coin_futures_kline_socket,
    get_usdt_futures_kline_socket)
from data_center.bmac.util import convert_interval_to_timedelta
from data_center.bmac.util.time import now_time


def convert_to_dataframe(x, interval_delta):

    columns = [
        'candle_begin_time', 'open', 'high',
        'low', 'close', 'volume',
    ...
    return pd.DataFrame(data=[candle_data],
                        columns=columns,
                        index=[candle_data[0] + interval_delta])
