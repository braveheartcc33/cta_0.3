from data_center.bmac.util import is_leverage_token, STABLECOINS
...
class TradingCoinPerpFilter:
    ...
    def is_trading_coin_swap(cls, x):
        ...
        '''
        return (x['quote_asset'] == 'USD' and
                x['status'] == 'TRADING' and
                x['contract_type'] == 'PERPETUAL')

    def __call__(self, syminfo: dict) -> list:
        symbols = [info['symbol'] for info in syminfo.values()
                   if self.is_trading_coin_swap(info)]
...
class TradingUsdtSpotFilter:
    ...
    def __call__(self, syminfo: dict) -> list:
        symbols = [info['symbol'] for info in syminfo.values()
                   if self.is_trading_usdt_spot(info)]
        if self.keep_symbols is not None:  # 如有白名单，则只保留白名单内的
            symbols = [sym for sym in symbols if sym in self.keep_symbols]
        return symbols
...
class TradingUsdtPerpFilter:
    ...
    def is_trading_usdt_swap(cls, x):
        ...
        '''
        return (x['quote_asset'] == 'USDT' and
                x['status'] == 'TRADING' and
                x['contract_type'] == 'PERPETUAL')

    def __call__(self, syminfo: dict) -> list:
        symbols = [info['symbol'] for info in syminfo.values()
                   if self.is_trading_usdt_swap(info)]
        if self.keep_symbols is not None:  # 如有白名单，则只保留白名单内的
            symbols = [sym for sym in symbols if sym in self.keep_symbols]
        return symbols
