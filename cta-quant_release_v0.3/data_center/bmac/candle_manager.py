...
class CandleFileManager:
    ...
    def __init__(self, base_dir, save_type):
        ...
        if save_type == 'feather':
            ...
            raise ValueError(f'Save type {save_type} not supported,'
                             'can only accept feather and parquet')
    ...
    def set_candle(self, symbol, run_time, df: pd.DataFrame):
        ...
        old_ready_file_paths = glob(os.path.join(self.base_dir,
                                                 f'{symbol}_*.ready'))
    ...
    def update_candle(self,
        ...
        df.drop_duplicates(subset='candle_begin_time',
