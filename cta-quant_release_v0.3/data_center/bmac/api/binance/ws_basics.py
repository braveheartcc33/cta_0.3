...
from data_center.bmac.api.binance.exceptions import (
    BinanceWebsocketUnableToConnect)
from data_center.bmac.util import get_loop
...
class ReconnectingWebsocket:
    ...
    async def connect(self):
        ...
        # To manage the "cannot call recv while
        # another coroutine is already waiting for the next message"
        if not self._handle_read_loop:
            ...
            while True:
                try:
                    ...
                    if self.ws_state == WSListenerState.EXITING:
                        self._log.debug(f"_read_loop {self._path} \
                                        break for {self.ws_state}")
                        break
                        ...
                        assert self.ws
                        res = await asyncio.wait_for(self.ws.recv(),
                        ...
                        if res:
                            if self._queue.qsize() < self.MAX_QUEUE_SIZE:
                                ...
                                self._log.debug(f"Queue overflow \
                                                {self.MAX_QUEUE_SIZE}. \
                                                    Message not filled")
    ...
    async def _run_reconnect(self):
        ...
        if self._reconnects < self.MAX_RECONNECTS:
            ...
            self._log.debug(f"websocket reconnecting."
            ...
            self._log.error(f"Max reconnections {self.MAX_RECONNECTS}"
                            f"reached:")
            # Signal the error
            await self._queue.put(
                {
    ...
    async def _wait_for_reconnect(self):
        while (self.ws_state != WSListenerState.STREAMING and
                self.ws_state != WSListenerState.EXITING):
            await asyncio.sleep(0.1)
