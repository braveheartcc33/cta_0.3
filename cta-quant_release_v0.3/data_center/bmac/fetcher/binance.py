...
from data_center.bmac.api.binance import create_binance_market_api
...
def _parse_usdt_futures_syminfo(info):
    ...
    return {
        ...
        'price_tick': Decimal(_get_from_filters(
            filters, 'PRICE_FILTER', 'tickSize')),
        'lot_size': Decimal(_get_from_filters(
            filters, 'LOT_SIZE', 'stepSize')),
        'min_notional_value': Decimal(_get_from_filters(
...
class BinanceFetcher:
    ...
    def get_api_limits(self) -> tuple[int, int]:
        return (self.market_api.MAX_MINUTE_WEIGHT,
                self.market_api.WEIGHT_EFFICIENT_ONCE_CANDLES)

    async def get_time_and_weight(self) -> tuple[pd.Timestamp, int]:
        server_timestamp, weight = await self.market_api. \
            aioreq_time_and_weight()
        server_timestamp = pd.to_datetime(server_timestamp, unit='ms',
                                          utc=True)
    ...
    async def get_exchange_info(self) -> dict[str, dict]:
        ...
        """
        exg_info = await async_retry_getter(
            self.market_api.aioreq_exchange_info)
        results = dict()
    ...
    async def get_candle(self, symbol, interval, **kwargs) -> pd.DataFrame:
        ...
        '''
        data = await async_retry_getter(self.market_api.aioreq_klines,
        ...
        df['candle_begin_time'] = pd.to_datetime(
            df['candle_begin_time'].astype('int64'),
            unit='ms', utc=True
        )
        for col in [
        ...
        df['candle_end_time'] = df['candle_begin_time'
                                   ] + convert_interval_to_timedelta(interval)
