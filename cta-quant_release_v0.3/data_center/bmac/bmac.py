...
from data_center.bmac.fetcher import BinanceFetcher
from data_center.bmac.msg_sender.dingding import DingDingSender
from data_center.bmac.util import (DEFAULT_TZ,
...
async def fetch_and_save_history_candle(interval,
                                        candle_mgr: CandleFileManager,
    ...
    if end_timestamp is None:
        candles = await fetcher.get_candle(symbol, interval,
                                           limit=once_candles)
    else:
        candles = await fetcher.get_candle(symbol, interval,
                                           limit=once_candles,
    ...
    df_candle = candle_mgr.update_candle(symbol, run_time,
                                         candles,
                                         num_candles)
...
async def init_history(handler: BmacHandler, session: aiohttp.ClientSession):
    ...
    logging.info('Exchange info data dir %s, initializing',
                 exginfo_mgr.base_dir)
    exginfo_mgr.clear_all()

    # 1. 通过调用 fetcher.get_exchange_info 获取所有交易的 symbol,
    # 并根据 symbol_filter 过滤出我们想要的 symbol
    exginfo = await fetcher.get_exchange_info()
    symbols_trading: list = handler.symbol_filter(exginfo)

    infos_trading = [info for sym, info in exginfo.items()
                     if sym in symbols_trading]
    ...
    while symbols_trading:
        ...
        cnt += 1
        logging.info('Round %d, Server time: %s, Used weight: %d, '
                     'Symbol num %d, %s - %s', cnt,
                     str(server_time.tz_convert(DEFAULT_TZ)), weight,
                     len(symbols_trading), fetch_symbols[0],
                     fetch_symbols[-1])
        ...
        for symbol in fetch_symbols:
            ...
            if symbol in last_begin_time:
                end_timestamp = (last_begin_time[symbol] - interval_delta
                                 ).value // 1000000
            t = fetch_and_save_history_candle(handler.interval,
                                              candle_mgr,
                                              fetcher,
                                              symbol,
                                              handler.num_candles,
        ...
        for symbol, (not_enough, begin_time, num
                     ) in zip(fetch_symbols, results):
            ...
            if num >= handler.num_candles or not_enough:
                ...
                if num < handler.num_candles:
                    logging.warn('%s finished not enough, candle num: %d',
                                 symbol, num)
                else:
    ...
    server_time, weight = await fetcher.get_time_and_weight()
    logging.info('Init history finished, Server time: %s, Used weight: %d',
                 str(server_time.tz_convert(DEFAULT_TZ)),
                 weight)
    return run_time
...
def check_candle(handler: BmacHandler,
    ...
    if not_readys:
        if report:
            logging.warning('Symbols not ready for runtime %s %s',
                            run_time, str(not_readys))
            for symbol in not_readys:
                rest_que.put_nowait({'run_time': run_time, 'symbol': symbol})
        else:
            logging.info('Run time %s, %d/%d symbols ready',
                         run_time,
                         len(symbols) - len(not_readys),
                         len(symbols))
        return False
...
def update_candle_data(handler: BmacHandler,
                       df_new: pd.DataFrame,
                       rest_que: asyncio.Queue,
                       symbol,
                       run_time):
    ...
    if min_new_begin_time - max_old_begin_time <= interval_delta:
        candle_mgr.update_candle(symbol, run_time.astimezone(DEFAULT_TZ),
                                 df_new, handler.num_candles)
    else:
        rest_que.put_nowait({'run_time': run_time, 'symbol': symbol})


async def dispatcher(handler: BmacHandler,
                     fetcher: BinanceFetcher,
                     senders: dict[str, DingDingSender],
                     listeners: list[CandleListener],
    ...
    while True:
        ...
        if req_type == 'update_exginfo':
            ...
            await update_exginfo(handler, fetcher, senders,
                                 listeners, run_time)
...
async def update_exginfo(handler: BmacHandler,
    ...
    # 2. 保存过滤出的 exginfo
    infos_trading = [info for sym, info in syminfo.items()
                     if sym in symbols_trading]
...
async def update_funding_fee(handler: BmacHandler,
                             fetcher: BinanceFetcher,
                             run_time):
    exginfo_mgr = handler.exginfo_mgr
    # 获取资金费率
...
async def fetch_recent_closed_candle(handler: BmacHandler,
                                     fetcher: BinanceFetcher,
                                     symbol,
                                     run_time):
    '''
    ...
    is_closed = False
    while True:
        df = await fetcher.get_candle(symbol, interval, limit=499)
        ...
        if now_time() - run_time > pd.Timedelta(seconds=expire_sec):
            # logging.warning(f'Candle may not closed in {expire_sec}'
            # f'sec {symbol} {interval}')
            break
...
async def restful_candle_fetcher(handler: BmacHandler,
                                 fetcher: BinanceFetcher,
                                 main_que: asyncio.Queue,
                                 rest_que: asyncio.Queue):
    while True:
        ...
        run_time = req['run_time']
        symbol = req['symbol']
        logging.warning('Fetch candle with restful API %s %s',
                        symbol, run_time)
        df_new, is_closed = await fetch_recent_closed_candle(handler, fetcher,
                                                             symbol, run_time)
        await main_que.put({
            'type': 'candle_data',
            'data': df_new,
...
def create_listeners(trade_type, time_interval, symbols, n_groups, main_que
                     ) -> list[CandleListener]:
    ...
    listeners = [CandleListener(trade_type, syms_grp, time_interval, main_que)
                 for syms_grp in groups]
    return listeners


async def update_candle(handler: BmacHandler,
                        session: aiohttp.ClientSession,
                        last_complete_run_time):
    '''
    ...
    rest_fetchers = [
        restful_candle_fetcher(handler, fetcher, main_que, rest_que)
        for _ in range(handler.num_rest_fetchers)
    ]

    symbols = handler.candle_mgr.get_all_symbols()
    listeners = create_listeners(handler.trade_type, handler.interval, symbols,
                                 handler.num_socket_listeners, main_que)
    listen_tasks = [listener.start_listen() for listener in listeners]

    alarm_task = period_alarm(handler, main_que)
    dispatcher_task = dispatcher(handler, fetcher, senders, listeners,
                                 main_que, rest_que, last_complete_run_time)
...
async def main(base_dir):
    # 读取 config.json，获取配置
    print(os.path.isfile(os.path.join(base_dir, 'bmac_config.json')))
    cfg = json.load(open(os.path.join(base_dir, 'bmac_config.json')))

    handler = BmacHandler(base_dir, cfg)

    logging.info('interval=%s, type=%s, funding_rate=%r, keep_symbols=%r',
                 handler.interval, handler.trade_type,
                 handler.fetch_funding_rate, handler.keep_symbols)

    while True:
        try:
            async with create_aiohttp_session(
                        handler.http_timeout_sec) as session:
...
async def report_error(handler: BmacHandler, e: Exception):
    ...
    if handler.dingding is not None and 'error' in handler.dingding:
        ...
        try:
            ...
            async with create_aiohttp_session(
                        handler.http_timeout_sec) as session:
                msg_sender = DingDingSender(session, dingding_err['secret'],
                                            dingding_err['access_token'])
